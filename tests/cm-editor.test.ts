...
test.describe('CMEditor (linting)', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('/lint-editor.html');
    await page.waitForSelector('.cm-editor');
    // Wait for linter to run - linter has a delay
    await page.waitForTimeout(500);
    // Wait for lint markers to appear
    await page.waitForSelector('.cm-lintRange', { timeout: 5000 }).catch(() => {});
  });

  test.describe('lint locators', () => {
    test('lintGutter returns gutter element', async ({ page }) => {
      const editor = CMEditor.from(page);
      await expect(editor.lintGutter).toHaveClass(/cm-gutter-lint/);
    });

    test('lintMarkers returns markers in gutter', async ({ page }) => {
      const editor = CMEditor.from(page);
      // Should have markers for error, warning, info, hint lines
      const count = await editor.lintMarkers.count();
      expect(count).toBeGreaterThan(0);
    });

    test('lintRanges returns lint range decorations', async ({ page }) => {
      const editor = CMEditor.from(page);
      const count = await editor.lintRanges.count();
      expect(count).toBeGreaterThan(0);
    });

    test('lintRangesOfSeverity filters by severity', async ({ page }) => {
      const editor = CMEditor.from(page);
      const errorCount = await editor.lintRangesOfSeverity('error').count();
      const warningCount = await editor.lintRangesOfSeverity('warning').count();
  ...
  test.describe('lint query methods', () => {
    test('diagnosticCounts returns counts by severity', async ({ page }) => {
      const editor = CMEditor.from(page);
      const counts = await editor.diagnosticCounts();

      expect(counts).toHaveProperty('error');
      expect(counts).toHaveProperty('warning');
      expect(counts).toHaveProperty('info');
      expect(counts).toHaveProperty('hint');
      expect(counts.error).toBeGreaterThan(0);
    });
  });

  test.describe('lint matchers', () => {
    test('toHaveDiagnosticCount checks total diagnostics', async ({ page }) => {
      const editor = CMEditor.from(page);
      // The lint editor has 4+ diagnostic lines
      const count = await editor.lintRanges.count();
      await expect(editor).toHaveDiagnosticCount(count);
    });
  });
});

test.describe('CMEditor (search)', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('/search-editor.html');
    await page.waitForSelector('.cm-editor');
  });
  ...
  test.afterEach(async ({ page }) => {
    ...
    const editor = CMEditor.from(page);
    if (await editor.isSearchPanelOpen()) {
      await editor.closeSearch();
    }
  });

  test.describe('search locators', () => {
    test('searchPanel returns search panel element', async ({ page }) => {
      const editor = CMEditor.from(page);
      await editor.openSearch();
      await expect(editor.searchPanel).toBeVisible();
    });

    test('searchInput returns search input field', async ({ page }) => {
      const editor = CMEditor.from(page);
      await editor.openSearch();
      await expect(editor.searchInput).toBeVisible();
    });

    test('replaceInput returns replace input field', async ({ page }) => {
      const editor = CMEditor.from(page);
      await editor.openSearch();
      await expect(editor.replaceInput).toBeVisible();
    });

    test('searchMatches returns highlighted matches', async ({ page }) => {
      const editor = CMEditor.from(page);
      await editor.search('hello');
      const count = await editor.searchMatches.count();
      expect(count).toBeGreaterThan(0);
    });
  });

  test.describe('search query methods', () => {
    test('isSearchPanelOpen returns false when closed', async ({ page }) => {
      const editor = CMEditor.from(page);
      const isOpen = await editor.isSearchPanelOpen();
      expect(isOpen).toBe(false);
    });

    test('isSearchPanelOpen returns true when open', async ({ page }) => {
      const editor = CMEditor.from(page);
      await editor.openSearch();
      const isOpen = await editor.isSearchPanelOpen();
    ...
    test('searchMatchCount returns number of matches', async ({ page }) => {
      const editor = CMEditor.from(page);
      await editor.search('hello');
      const count = await editor.searchMatchCount();
      expect(count).toBeGreaterThan(0);
    });
  });

  test.describe('search action methods', () => {
    test('openSearch opens the search panel', async ({ page }) => {
      const editor = CMEditor.from(page);
      await editor.openSearch();
      await expect(editor.searchPanel).toBeVisible();
    });

    test('closeSearch closes the search panel', async ({ page }) => {
      const editor = CMEditor.from(page);
      await editor.openSearch();
      await editor.closeSearch();
      await expect(editor.searchPanel).not.toBeVisible();
    });

    test('search fills search input and highlights matches', async ({ page }) => {
      const editor = CMEditor.from(page);
      await editor.search('world');
      await expect(editor.searchInput).toHaveValue('world');
      // "world" appears 5 times in the test fixture
      await expect(editor).toHaveSearchMatches(5);
    });

    test('search with replaceWith fills both fields', async ({ page }) => {
      const editor = CMEditor.from(page);
      await editor.search('hello', { replaceWith: 'goodbye' });
      await expect(editor.searchInput).toHaveValue('hello');
      await expect(editor.replaceInput).toHaveValue('goodbye');
    });
  });

  test.describe('search matchers', () => {
    test('toHaveSearchMatches checks match count', async ({ page }) => {
      const editor = CMEditor.from(page);
      await editor.search('hello');
      // "hello" appears 6 times in the search test code
      await expect(editor).toHaveSearchMatches(6);
    });

    test('toHavePanelOpen detects open search panel', async ({ page }) => {
      const editor = CMEditor.from(page);
      await editor.openSearch();
      await expect(editor).toHavePanelOpen();
    });
  });
});

test.describe('CMEditor (tooltips)', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('/tooltip-editor.html');
    await page.waitForSelector('.cm-editor');
  });

  test.describe('tooltip locators', () => {
    test('tooltips returns tooltip elements', async ({ page }) => {
      const editor = CMEditor.from(page);
      // Hover over "function" keyword to trigger tooltip
      const line = editor.lineInDOMContaining('function example');
      await line.hover();
      await page.waitForTimeout(300); // Wait for hover delay
      // Tooltip may or may not appear depending on exact hover position
      const count = await editor.tooltips.count();
      expect(count).toBeGreaterThanOrEqual(0);
    });
  });

  test.describe('tooltip query methods', () => {
    test('tooltipCount returns number of tooltips', async ({ page }) => {
      const editor = CMEditor.from(page);
      // Initially no tooltips
      const count = await editor.tooltipCount();
      expect(count).toBe(0);
    });

    test('hasVisibleTooltip returns false when no tooltip', async ({ page }) => {
      const editor = CMEditor.from(page);
      const hasTooltip = await editor.hasVisibleTooltip();
      expect(hasTooltip).toBe(false);
    });
  });
});

test.describe('CMEditor (bracket matching)', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('/bracket-editor.html');
    await page.waitForSelector('.cm-editor');
  });

  test.describe('bracket locators', () => {
    test('matchingBrackets returns matching bracket elements', async ({ page }) => {
      const editor = CMEditor.from(page);
      // Click on the first line to position cursor
      await editor.content.click();

      // Use arrow keys to move to a bracket - the code starts with "function example() {"
      // Navigate to the opening brace
      await editor.view.press('End'); // Go to end of first line
      await page.waitForTimeout(100);

      // Check if any brackets are highlighted (depends on cursor position)
      const count = await editor.matchingBrackets.count();
      // May or may not find matches depending on cursor position
      expect(count).toBeGreaterThanOrEqual(0);
    });
  });

  test.describe('bracket query methods', () => {
    test('hasBracketMatch returns boolean', async ({ page }) => {
      const editor = CMEditor.from(page);
      const hasMatch = await editor.hasBracketMatch();
      expect(typeof hasMatch).toBe('boolean');
    });

    test('hasNonMatchingBracket returns boolean', async ({ page }) => {
      const editor = CMEditor.from(page);
      const hasNonMatch = await editor.hasNonMatchingBracket();
      expect(typeof hasNonMatch).toBe('boolean');
    });

    test('matchingBracketChars returns array', async ({ page }) => {
      const editor = CMEditor.from(page);
      const chars = await editor.matchingBracketChars();
      expect(Array.isArray(chars)).toBe(true);
    });
  });
});

test.describe('CMEditor (panels)', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('/search-editor.html');
    await page.waitForSelector('.cm-editor');
  });

  test.describe('panel locators', () => {
    test('panels returns panel elements', async ({ page }) => {
      const editor = CMEditor.from(page);
      await editor.openSearch();
      const count = await editor.panels.count();
      expect(count).toBeGreaterThan(0);
    });

    test('panelsAt returns panels at specific position', async ({ page }) => {
      const editor = CMEditor.from(page);
      await editor.openSearch();
      // Search panel is typically at top
      const topCount = await editor.panelsAt('top').count();
      expect(topCount).toBeGreaterThanOrEqual(0);
    });
  });

  test.describe('panel query methods', () => {
    test('panelCount returns number of panels', async ({ page }) => {
      const editor = CMEditor.from(page);
      const initialCount = await editor.panelCount();
      expect(initialCount).toBe(0);

      await editor.openSearch();
      const afterCount = await editor.panelCount();
      expect(afterCount).toBeGreaterThan(0);
    });

    test('hasPanelOpen returns true when panel is open', async ({ page }) => {
      const editor = CMEditor.from(page);
      expect(await editor.hasPanelOpen()).toBe(false);

      await editor.openSearch();
      expect(await editor.hasPanelOpen()).toBe(true);
    });

    test('hasPanelAt checks panel at specific position', async ({ page }) => {
      const editor = CMEditor.from(page);
      await editor.openSearch();
      // hasPanelAt returns true if panel exists at that position
      const hasTop = await editor.hasPanelAt('top');
      const hasBottom = await editor.hasPanelAt('bottom');
      // At least one should be true (search panel is open)
